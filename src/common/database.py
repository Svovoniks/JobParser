...
class DataBase(metaclass=Singleton):
    ...
    def get_resumes(self, resume: ResumeRequest) -> List[Resume]:
        ...
        if resume['gender'] !=  None:
            select_statement =  select_statement.where(resume_table.c.gender == resume['gender'])
        ...
        if resume['search_status'] != None:
            select_statement = select_statement.where(resume_table.c.search_status == resume['search_status'])
    ...
    def get_vacancies(self, vacancy: VacancyRequest) -> List[Vacancy]:
        ...
        if vacancy['experience'] !=  None:
            select_statement =  select_statement.where(vacancy_table.c.experience == vacancy['experience'])
        
        if vacancy['employer'] !=  None:
            select_statement =  select_statement.where(vacancy_table.c.employer == vacancy['employer'])
        ...
        if vacancy['area'] != None:
            select_statement = select_statement.where(vacancy_table.c.area == vacancy['area'])
    ...
    def get_all_options(self, column: Column):
        return set(map(lambda a: a[0], self.execute(select(column))))
